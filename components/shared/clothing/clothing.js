...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('close-product-popup').addEventListener('click', function () {
  ...
  const pageNameWithoutExtension = lastSegment.substring(0, lastSegment.lastIndexOf('.'));
  const genderFilterField = pageNameWithoutExtension === 'men' ? 'Male' : 'Female';
  $.ajax({
    ...
    success: function (data) {
      ...
      filterFieldsArr.forEach(filterField => {
        insertAvailableFilterField(data, filterField.fieldName, filterField.filterContainer);
      });
    },
...
function insertProductsHtml(data) {
  ...
  $.each(data, function (index, product) {
    ...
    productsHTML += '  <img src="' + product.imagePath + '" alt="' + product.name + '">';
    productsHTML += ' </div>';
